...
function actions(r){const p=policy(r);return `<div class="row-actions">${Object.entries(p).map(([a,reason])=>`<span title="${reason}"><button class="btn-text" data-action="${a}" data-id="${r.id}" ${reason?'disabled':''}>${a}</button></span>`).join('')}</div>`;}
...
function render(){
 filtered=records.filter(r=>
  ...
  inDateRange(r.vestingDate,$('#filterVestingStart').value,$('#filterVestingEnd').value)&&
 ...
 $('#rows').innerHTML=filtered.slice((page-1)*10,page*10).map(r=>`<tr data-row="${r.id}">${keys.map(k=>`<td class="${k==='dept'?'department':''}" title="${esc(r[k])}">${k==='name'?`<button class="btn-text" data-detail="${r.id}">${esc(r.name)}</button>`:k==='status'?`<span class="cell-with-icon">${badge(r.status)}${r.history?.length>1?historyButton(r):''}</span>`:k==='punchStatus'?`<span class="cell-with-icon">${r.punchStatus==='—'?'—':badge(r.punchStatus)+historyButton(r,true)}</span>`:esc(r[k])}${k==='start'&&r.groups?.length>1?` <span class="tag">${r.groups.length}组</span>`:''}</td>`).join('')}<td>${actions(r)}</td></tr>`).join('')||`<tr><td colspan="${keys.length+1}" class="empty">暂无符合条件的加班申请</td></tr>`;
...
function showHistory(r,punch){const entries=(punch?r.punchHistory:r.history.slice(1)).map((record,index)=>({record,index:punch?index:index+1}));modal(punch?'补卡历史':'历史记录',`<p class="muted" style="margin-bottom:16px">${esc(r.name)} · ${esc(r.no)}</p><table><thead><tr><th>单据类型</th><th>提交时间</th><th>审批状态</th><th>操作</th></tr></thead><tbody>${entries.map(({record:x,index})=>`<tr><td>${x.type}</td><td>${x.submitted}</td><td>${badge(x.status)}</td><td><button class="btn-text" data-version="${index}" data-id="${r.id}" data-punch="${punch}">查看</button></td></tr>`).join('')||'<tr><td colspan="4" class="empty">暂无历史记录</td></tr>'}</tbody></table>`);}
...
$('#export').onclick=()=>{const csv=v=>'"'+String(v??'').replace(/^[=+@-]/,"'$&").replaceAll('"','""')+'"';const data=[exportFields,...filtered.map(r=>exportFields.map(f=>r[exportableKeys[exportableFields.indexOf(f)]]))].map(row=>row.map(csv).join(',')).join('\r\n');const url=URL.createObjectURL(new Blob(['\ufeff'+data],{type:'text/csv;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download='加班申请.csv';a.click();setTimeout(()=>URL.revokeObjectURL(url),500);};
if(isSelf){
