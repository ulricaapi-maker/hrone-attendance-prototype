...
(function(){
 ...
 function enforce(){
  $('#employee').disabled=true;$('#addGroup').hidden=cancel;document.querySelector('.group-add-row').hidden=cancel;document.querySelectorAll('[data-remove]').forEach(b=>{b.hidden=cancel;b.disabled=cancel;});
  document.querySelectorAll('[data-key="reason"]').forEach(el=>{el.readOnly=cancel;const field=el.closest('.form-item');field.querySelector('.form-label').textContent=cancel?'原申请理由':'申请理由';field.hidden=false;});
 ...
 $('#application').onsubmit=e=>{
  ...
  const updatedGroups=cancel?structuredClone(originalGroups):groups.map((group,index)=>{const saved=structuredClone(group);saved.detailId=group.originDetailId||group.detailId||`${target.id}-N${index+1}`;saved.hours=duration(group)===null?'—':Number(duration(group)).toFixed(2);delete saved.originDetailId;delete saved.id;delete saved.collapsed;delete saved.vestingBasis;return saved;}),g=updatedGroups[0];
